...
public class ChessGame {
        ...
        for (int i = 1; i < 9; i++) {
            for (int j = 1; j < 9; j++) {
                ...
                if (gameBoard.hasPiece(currentPosition) && !gameBoard.getPiece(currentPosition).getTeamColor().equals(teamColor)) {
                    ...
                    if(checkIfKingAttacked(possibleMoves, teamColor)){
                        return true;
                    }
                }
            }
    ...
    public boolean checkIfKingAttacked(ArrayList<ChessMove> possibleMoves, TeamColor teamColor) {
        for (ChessMove possibleMove : possibleMoves) {
            ChessPosition currentEndPos = possibleMove.getEndPosition();
    ...
    public boolean isInCheckmate(TeamColor teamColor) {
        ...
        for (int i = 1; i < 9; i++) {
            for (int j = 1; j < 9; j++) {
                ChessPosition currentPosition = new ChessPosition(i, j);
                if (!(gameBoard.hasPiece(currentPosition) && gameBoard.getPiece(currentPosition).getTeamColor().equals(teamColor))) {
                    //If we're a nonexistent piece or not on our team, move on to try to find a teammate
                    continue;
                }
                ...
                for (ChessMove tryingMove : possibleMoves) {
                    ChessBoard backupBoard = gameBoard.duplicate();
                    try {
                        makeMove(tryingMove);
